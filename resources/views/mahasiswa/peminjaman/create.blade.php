...
@section('content')
    ...
    <div class="page-header">
        <div>
            ...
            <p>Isi formulir berikut untuk mengajukan peminjaman</p>
        </div>
    </div>
        ...
        <form action="{{ route('mahasiswa.peminjaman.store') }}" method="POST" id="peminjamanForm">
            ...
            <div class="field-group">
                ...
                <div style="display:flex;gap:16px">
                    <label style="display:flex;align-items:center;gap:8px;cursor:pointer;font-size:13.5px;font-weight:400">
                        <input type="radio" name="jenis" id="jenisLab" value="lab" @checked(old('jenis', 'lab') === 'lab')
                            onchange="toggleJenis(this.value)"> Ruang Lab
                    </label>
                    <label style="display:flex;align-items:center;gap:8px;cursor:pointer;font-size:13.5px;font-weight:400">
                        <input type="radio" name="jenis" value="barang" @checked(old('jenis') === 'barang')
                            onchange="toggleJenis(this.value)"> Barang / Alat
                    </label>
                </div>
            ...
            <div id="section_lab">
                <div class="field-group">
                    ...
                    <select id="namaLabSelect" name="nama_lab" onchange="onLabChange()">
                        ...
                        @foreach($labs as $lab)
                            <option value="{{ $lab->nama_lab }}" data-stok="{{ $lab->stok }}"
                                data-kursi="{{ $lab->jumlah_kursi }}" @selected(old('nama_lab') === $lab->nama_lab)>
                ...
                <div class="field-row">
                    <div class="field-group">
                        ...
                        <select id="id_barang" name="id_barang" onchange="setBarangHidden(this)">
                            ...
                            @foreach($barang as $b)
                                <option value="{{ $b->id_barang }}" data-nama="{{ $b->nama_barang }}" data-stok="{{ $b->stok }}"
                                    data-lab="{{ $b->lab->nama_lab ?? '' }}" @selected(old('id_barang') == $b->id_barang)>
            ...
            <div class="field-row">
                <div class="field-group">
                    ...
                    <input type="date" id="tanggalInput" name="tanggal" value="{{ old('tanggal') }}" required
                        min="{{ date('Y-m-d') }}" onchange="onScheduleChange()">
            ...
            <div class="field-group">
                ...
                <div style="display:flex;gap:8px;align-items:center">
                    <input type="time" id="jamMulaiInput" name="jam_mulai" value="{{ old('jam_mulai') }}" required
                    ...
                    <input type="time" id="jamSelesaiInput" name="jam_selesai" value="{{ old('jam_selesai') }}" required
                        onchange="onScheduleChange()"
                        style="height:38px;padding:0 12px;border:1px solid var(--border);border-radius:8px;font-family:DM Sans,sans-serif;font-size:13.5px;flex:1;outline:none">
                </div>
            ...
            @error('kursi')<span class="field-error"
            style="display:block;margin-top:-12px;margin-bottom:12px">{{ $message }}</span>@enderror

            @if($errors->any())
                <div class="alert-error">
                    @foreach($errors->all() as $err)
                    <div>{{ $err }}</div>@endforeach
                </div>
            @endif
    ...
    @push('scripts')
        <script>
            ...
            function setBarangHidden(sel) {
                ...
                // Set limit max berdasarkan stok
